...
class ItemServiceImplTest {
    @Mock
    private IItemRepository groupsRepository;
    private IItemService underTest;

    //перед каждым тестом получаем чистые данные
    @BeforeEach
    public void setUp() {
        underTest = new ItemServiceImpl(groupsRepository);
    }

    @Test
    void createEntity() {
        // given
        Item groups = new Item();
        groups.setItemName("Собака");
        ...
        ArgumentCaptor<Item> groupsArgumentCaptor =
        ...
        Item groupsArgumentCaptorValue = groupsArgumentCaptor.getValue();
